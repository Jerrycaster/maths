...
\label{sec:2.1}
  ...
  \begin{definition}[Quotient topology]
    Let $(X,\tau_X)$ be a topological space, and $\sim$ an equivalence relation on $X$. 
    We define the \textit{natural map}, $p:X\to\X$, by $p(x)=[x]$ $\forall x\in X$. \\

    Then $(\X,\tau)$ is a topological space, where the \textit{quotient topology}, $\tau$, is defined by
    $$\tau=\{U\in \X:p^{-1}(U)\in\tau_X\}.$$
    Note that $\tau$ is finest topology on $\X$ such that the natural map $p$ is continuous.
...
\label{sec:2.2}
  ...
  We can think of a group action as a homomorphism $\psi:G\to\text{Aut}(X):g\mapsto\sigma$, where $\sigma(x)=g\cdot x$. 
  Then the action of $g\in G$ on $x\in X$ is equivalent to applying $\sigma$ to $x$.
...
\label{sec:2.6}
  ...
  \begin{definition}[Manifold]
    An \textit{manifold}, $X$, of dimension $n$ is a topological space with is locally homeomorphic to $\R^n$. 
    That is, $\forall x\in X$, there exists an open neighbourhood $U\subset X$ of $x$ such that there is a homeomorphism 
    $$f:U\to\R^n.$$
  \end{definition}
