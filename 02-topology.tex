...
\label{sec:2.2}
  ...
  \begin{enumerate}
    \item The identity transformation in $\Aut{X}$ is assigned to the identity in $G$,
    \item The composition of the permutations assigned to two elements $g,h\in G$ is assigned to 
          the product $gh$.
  \end{enumerate}
  Since $\text{Im}(\phi)$ is a subgroup of $\Aut{X}$, we define the action of $G$ on $X$ by 
  the group action of the homomorphic image of $G$, in $\Aut{X}$, on $X$. Specifically:
...
\label{sec:2.6}
  ...
  \begin{definition}[Chart]
    A \textit{chart}, $(U,\phi)$, for a topological space $X$ is an ordered pair of an open subset 
    $U\subset X$ to an open subset of $R^n$.
  \end{definition}

  \begin{definition}[Atlas]
    An \textit{atlas} of $X$ is a collection of charts $\{(U_i,\phi_i):i\in I\}$ such that
    $$\bigcup_{i\in I}{U_i}=X.$$
  \end{definition}

  \begin{definition}[Manifold]
    If $\phi_i(U_i)=\R^n$ $\forall i\in I$, then $X$ is called an n-dimensional \textit{manifold}.
  \end{definition}
